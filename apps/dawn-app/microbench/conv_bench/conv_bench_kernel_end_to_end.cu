...
std::tuple<int, int, int, std::string> time_cnn(
    ...
    size_t input_size = 0;
    store->get_direct(pool, key_name, input.begin(), input_size, handle);
    ...
    PLOG("Read from remote kvstore takes %lf ms", micro_sec/(double)num_repeats);
...
int main(int argc, char **argv) {
    ...
    // Create pool
    auto pool = store->create_pool("/pools", "gpu0", MB(128));
