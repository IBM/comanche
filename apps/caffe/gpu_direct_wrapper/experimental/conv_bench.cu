...
std::tuple<int, int, int, std::string> time_cnn(
    ...
    file_name = file_name+std::to_string(w)\
    		+"_h"+std::to_string(h)\
    		+"_c"+std::to_string(c)\
    		+"_n"+std::to_string(n)\
    		+"_k"+std::to_string(k)\
    ...
    write_to_file(file_name, random_input);
    put_into_kvstore(kvstore, file_name, random_input);

    //GDR Initialization.
    GDR_ptr gdr_ptr;
    gdr_mh_t gdr_mh;
    gdr_ptr._d_ptr = (CUdeviceptr) input.begin();
    if(gdr_pin_buffer(gdr->_gdr,
                      gdr_ptr._d_ptr,
                      input.size(),
                      0,
                      0,
    ...
    void * bar_ptr = NULL;
    if(gdr_map(gdr->_gdr, gdr_mh, &bar_ptr, input.size()), 0)
      throw General_exception("gdr_map failed");
    PLOG("gdr_map returned bar_ptr=%p", bar_ptr);
    assert(bar_ptr);

    gdr_info_t info;
    if(gdr_get_info(gdr->_gdr, gdr_mh, &info))
        throw General_exception("gdr_get_info failed");
    int off = ((unsigned long)gdr_ptr._d_ptr) - info.va;
    PLOG("offset: %d", off);
    gdr_ptr._h_ptr = (void*) ((char*)gdr_ptr._h_ptr+off);
    /*
    ...
    new_vaddr = xms_mmap((void *)0x900000000, new_paddr, MB(8));

    PLOG("new paddr=0x%lx vaddr=0x%lx", (addr_t) new_paddr, (addr_t)new_vaddr);
    memset(new_vaddr, 0x0, input.size());
    Component::io_buffer_t iob = memory_instance.register_memory_for_io(new_vaddr, new_paddr, MB(8));
    ...
    */
    ...
    PLOG("Read from file takes %lf ms", micro_sec/(double)num_repeats);
    ...
    */
    ...
    if(gdr_unmap(gdr->_gdr, gdr_mh, bar_ptr, MB(32)), 0)
         throw General_exception("gdr_unmap failed");
    if(gdr_unpin_buffer(gdr->_gdr, gdr_mh))
        	throw General_exception("gdr_unpin_buffer failed");
