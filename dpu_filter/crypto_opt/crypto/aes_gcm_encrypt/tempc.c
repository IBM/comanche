...
#define MAX_BUFFER_SIZE 2097128  // Define the maximum buffer size, 2MB
...
aes_gcm_encrypt(struct aes_gcm_cfg *cfg, char *file_data, size_t file_size)
{
    ...
    /*Need to change this so that we divide this in chunks*/
    ...
    size_t num_chunks = (file_size + MAX_BUFFER_SIZE - 1) / MAX_BUFFER_SIZE;
	DOCA_LOG_INFO("File was encrypted successfully and saved in: %zu", num_chunks);
    size_t total_buffer_size = (cfg->iv_length + cfg->tag_size) * num_chunks + file_size;
    DOCA_LOG_INFO("File was encrypted successfully and saved in: %zu", total_buffer_size);


	
    // Allocate a single large buffer based on the larger of the two sizes
    dst_buffer = calloc(1, max_encrypt_buf_size*num_chunks);
...
/////////////////////////////////////////////////////////////////////////////////////////
	result = doca_mmap_set_memrange(state->dst_mmap, dst_buffer, max_encrypt_buf_size*num_chunks);
	if (result != DOCA_SUCCESS) {
...
////////////////////////////////////////////////////////////////////////////////////////
    ...
    while (offset < file_size) {
        size_t current_chunk_size = (file_size - offset > MAX_BUFFER_SIZE) ? MAX_BUFFER_SIZE : file_size - offset;

      DOCA_LOG_INFO("chunk: %zu", current_chunk_size);
	        DOCA_LOG_INFO("offset: %zu", offset);

///////////////////////////////////////////////////////////////////////////////////////////
	/* Construct DOCA buffer for each address range */
	result = doca_buf_inventory_buf_get_by_addr(state->buf_inv, state->src_mmap,
						    file_data+offset, current_chunk_size-1, &src_doca_buf);
	...
	/* Construct DOCA buffer for each address range */
	result = doca_buf_inventory_buf_get_by_addr(state->buf_inv, state->dst_mmap, dst_buffer+d_offset, max_encrypt_buf_size,
						    &dst_doca_buf);
	if (result != DOCA_SUCCESS) {
