...
# Column names for specific TPC-DS tables based on their schema definitions
columns = {
    'customer_address': [
    ...
    'date_dim': [
        ...
        'd_day_name', 'd_quarter_name', 'd_holiday', 'd_weekend', 'd_following_holiday', 'd_first_dom',
        'd_last_dom', 'd_same_day_ly', 'd_same_day_lq', 'd_current_day', 'd_current_week',
        'd_current_month', 'd_current_quarter', 'd_current_year'
    ],
    'warehouse': [
        'w_warehouse_sk', 'w_warehouse_id', 'w_warehouse_name', 'w_warehouse_sq_ft', 'w_street_number',
        'w_street_name', 'w_street_type', 'w_suite_number', 'w_city', 'w_county', 'w_state', 'w_zip',
    ...
    'item': [
        ...
        'i_category', 'i_manufact_id', 'i_manufact', 'i_size', 'i_formulation', 'i_color', 'i_units',
        'i_container', 'i_manager_id', 'i_product_name'
    ],
    'store': [
        's_store_sk', 's_store_id', 's_rec_start_date', 's_rec_end_date', 's_closed_date_sk', 's_store_name',
        's_number_employees', 's_floor_space', 's_hours', 's_manager', 's_market_id', 's_geography_class',
        's_market_desc', 's_market_manager', 's_division_id', 's_division_name', 's_company_id',
        's_company_name', 's_street_number', 's_street_name', 's_street_type', 's_suite_number', 's_city',
        's_county', 's_state', 's_zip', 's_country', 's_gmt_offset', 's_tax_precentage'
    ],
    'call_center': [
        'cc_call_center_sk', 'cc_call_center_id', 'cc_rec_start_date', 'cc_rec_end_date', 'cc_closed_date_sk',
        'cc_open_date_sk', 'cc_name', 'cc_class', 'cc_employees', 'cc_sq_ft', 'cc_hours', 'cc_manager',
        'cc_mkt_id', 'cc_mkt_class', 'cc_mkt_desc', 'cc_market_manager', 'cc_division', 'cc_division_name',
        'cc_company', 'cc_company_name', 'cc_street_number', 'cc_street_name', 'cc_street_type',
        'cc_suite_number', 'cc_city', 'cc_county', 'cc_state', 'cc_zip', 'cc_country', 'cc_gmt_offset',
        'cc_tax_percentage'
    ],
    'customer': [
    ...
    'web_page': [
        ...
        'wp_access_date_sk', 'wp_autogen_flag', 'wp_customer_sk', 'wp_url', 'wp_type', 'wp_char_count',
        'wp_link_count', 'wp_image_count', 'wp_max_ad_count'
    ],
    'promotion': [
        'p_promo_sk', 'p_promo_id', 'p_start_date_sk', 'p_end_date_sk', 'p_item_sk', 'p_cost', 'p_response_target',
    ...
    'web_sales': [
        ...
        'ws_ext_ship_cost', 'ws_net_paid', 'ws_net_paid_inc_tax', 'ws_net_paid_inc_ship', 'ws_net_paid_inc_ship_tax',
        'ws_net_profit'
    ],
    'catalog_sales': [
...
for table in tables:
    ...
    # Read the .dat file
    df = pd.read_csv(file_path, delimiter='|', names=columns[table], index_col=False, usecols=range(len(columns[table])))

    # Ensure the number of columns read matches the number of columns specified
    if df.shape[1] != len(columns[table]):
        print(f"Warning: Number of columns in {table} does not match the specified column names.")
        print(f"Expected {len(columns[table])} columns but found {df.shape[1]}.")

    # Inspect the DataFrame to ensure all columns are present
    ...
    # Write table to Parquet with specified row group size and without compression
    pq_writer = pq.ParquetWriter(table_path, table.schema, use_dictionary=True, compression=None)
    for i in range(0, len(table), row_group_size):
        pq_writer.write_table(table.slice(i, row_group_size))
    pq_writer.close()

    # Read back the Parquet file to count the number of row groups
    parquet_file = pq.ParquetFile(table_path)
    num_row_groups = parquet_file.num_row_groups
    print(f"Table: {table}, Total Row Groups: {num_row_groups}")
