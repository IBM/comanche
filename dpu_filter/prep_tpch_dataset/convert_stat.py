...
def convert_to_parquet(file_path, columns, output_dir, row_group_size=100000):
    ...
    # Write Arrow table to Parquet file with specified row group size without compression and with statistics
    parquet_file_path = os.path.join(output_dir, f'{file_name}.parquet')
    pq.write_table(table, parquet_file_path, row_group_size=row_group_size, write_statistics=True)
...
output_dir = '/mnt/sda4/parquet_100K_sf1_stat'
...
print("All files converted to Parquet format with row group statistics.")