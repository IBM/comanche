        ...
        for (size_t i = 0; i < tokens.size(); ++i) {
            ...
            if (processingWhereClause) {
                if (tokens[i].type == TokenType::COLUMN) {
                    ...
                    // Handle int64 columns (non-nullable)
                    if (columnName == "ws_item_sk" || columnName == "ws_order_number") {
                        ...
                        // Handle nullable double columns
                        auto literal_value = arrow::compute::literal(std::stod(value));
                        auto is_not_null = arrow::compute::is_valid(field_ref);

                        if (operatorSymbol == "=") {
                            condition = arrow::compute::and_(is_not_null, arrow::compute::equal(field_ref, literal_value));
                        } else if (operatorSymbol == ">") {
                            condition = arrow::compute::and_(is_not_null, arrow::compute::greater(field_ref, literal_value));
                        } else if (operatorSymbol == ">=") {
                            condition = arrow::compute::and_(is_not_null, arrow::compute::greater_equal(field_ref, literal_value));
                        } else if (operatorSymbol == "<") {
                            condition = arrow::compute::and_(is_not_null, arrow::compute::less(field_ref, literal_value));
                        } else if (operatorSymbol == "<=") {
                            condition = arrow::compute::and_(is_not_null, arrow::compute::less_equal(field_ref, literal_value));
                        } else if (operatorSymbol == "!=") {
                            condition = arrow::compute::and_(is_not_null, arrow::compute::not_equal(field_ref, literal_value));
                        } else if (operatorSymbol == "BETWEEN") {
                            auto literal_value2 = arrow::compute::literal(std::stod(value2));
                            condition = arrow::compute::and_(
                                is_not_null,
                                arrow::compute::and_(
                                    arrow::compute::greater_equal(field_ref, literal_value),
                                    arrow::compute::less_equal(field_ref, literal_value2)
                                )
                            );
                        }
...
void filterDataSync(const arrow::compute::Expression& filter_expression, 
                    std::shared_ptr<arrow::io::BufferReader> bufferReader, 
                    int row_group_index, 
                    std::shared_ptr<arrow::Table>& final_result_table, 
    ...
    try {
        ...
        /*if (is_aggregation) {
            ...
            auto quantity_column = result_table->GetColumnByName("ws_quantity");
...
int main() {
    ...
    const std::string key = "enc_web_sales.parquet";
    ...
    int queryIndex = 1; // Initialize the query index

    for (const auto& sqlExpression : queries) {
        std::cout << "Processing SQL Statement: " << sqlExpression << std::endl;
            ...
            if (getObjectOutcome.IsSuccess()) {
                ...
                if (!decrypted_data.empty()) {
                    ...
                    for (int row_group_index = 0; row_group_index < num_row_groups; ++row_group_index) {
                        filterDataSync(filter_expression, bufferReader, row_group_index, final_table, select_columns, is_aggregation);
                    }
                    ...
                    // Calculate timings for Fetch, Decrypt, Filter, and Total for the current query
                    // These would be the respective durations you have calculated for each query iteration
                    double fetchTime = s3_fetch_duration.count();
                    double decryptTime = decrypt_elapsed.count();
                    double filterTime = table_duration.count();
                    double totalTime = total_duration.count();

                    if (isFirstQuery) {
                        // On the first query, write the headers and the first column of data
                        outputFile.open("../timing_results_web.txt");
                        if (outputFile.is_open()) {
                            outputFile << "Query #\t" << queryIndex << "\n";
                            outputFile << "Fetch\t" << std::fixed << std::setprecision(3) << fetchTime << "\n";
                            outputFile << "Decrypt\t" << std::fixed << std::setprecision(3) << decryptTime << "\n";
                            outputFile << "Filter\t" << std::fixed << std::setprecision(3) << filterTime << "\n";
                            outputFile << "Total\t" << std::fixed << std::setprecision(3) << totalTime << "\n";
                            outputFile.close();
                            isFirstQuery = false;
                        } else {
                        ...
                        // On subsequent queries, append new columns of data
                        ...
                        if (inputFile.is_open()) {
                            ...
                            if (outputFile.is_open()) {
                                ...
                                outputFile << lines[lineIndex++] << "\t" << std::fixed << std::setprecision(3) << fetchTime << "\n"; // Append Fetch time
                                outputFile << lines[lineIndex++] << "\t" << std::fixed << std::setprecision(3) << decryptTime << "\n"; // Append Decrypt time
                                outputFile << lines[lineIndex++] << "\t" << std::fixed << std::setprecision(3) << filterTime << "\n"; // Append Filter time
                                outputFile << lines[lineIndex++] << "\t" << std::fixed << std::setprecision(3) << totalTime << "\n"; // Append Total time
