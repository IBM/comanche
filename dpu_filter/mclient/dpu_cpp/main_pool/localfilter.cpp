...
size_t WriteCallback(void* contents, size_t size, size_t nmemb, void* userp) {
    ...
    chunk->data.resize(chunk->data.size() + totalSize);
    std::copy(static_cast<char*>(contents), static_cast<char*>(contents) + totalSize, chunk->data.end() - totalSize);
    return totalSize;
}
...
bool DownloadFileParallel(const std::string& url, size_t content_size, std::vector<char>& data) {
    const size_t chunk_size = 1024 * 1024 * 2 ; // 1 MB
    ...
    for (size_t i = 0; i < num_chunks; ++i) {
        ...
        if (chunks[i].end >= content_size) {
            chunks[i].end = content_size - 1;
        }
    }

    std::vector<std::future<bool>> futures;
    for (size_t i = 0; i < num_chunks; ++i) {
        futures.push_back(thread_pool.enqueue([&, i] {
    ...
    for (const auto& chunk : chunks) {
        std::copy(chunk.data.begin(), chunk.data.end(), data.begin() + offset);
