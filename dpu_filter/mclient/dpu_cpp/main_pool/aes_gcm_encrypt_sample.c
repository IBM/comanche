...
aes_gcm_encrypt(struct aes_gcm_cfg *cfg, char *file_data, size_t file_size)
{
	...
	buffer_size = MAX_BUFFER_SIZE - cfg->tag_size;
	...
	//if max_encrypt size != 2MB. update number of chunks etc,.

    
	/* Start AES-GCM context */
	result = doca_ctx_start(state->ctx);  // takes 222 msec
	if (result != DOCA_SUCCESS) {
    ...
    // Allocate a single large buffer based on the larger of the two sizes
...
////////////////////////////////////////////////////////////////////////////////////////
	...
	/*if (mlock(file_data, file_size) != 0) {
    	DOCA_LOG_INFO("can't mlock");
    	goto destroy_dst_buf;
	}

	    // Pin the destination buffer memory
	if (mlock(dst_buffer, max_encrypt_buf_size*num_chunks) != 0) {
		DOCA_LOG_INFO("Can't mlock");
		goto free_dst_buf;
	}*/
	...
	/* Construct DOCA buffer for each address range. Using a single dst buffer, but can use several dst buffers */
    ...
    double elapsed_ms = get_time(&start, &end);
...
///////////////////////////////////////////////////////////////////////////////////////////
		...
		result = doca_buf_inventory_buf_get_by_addr(state->buf_inv, state->src_mmap,
						    file_data+offset, current_chunk_size, &src_doca_buf);
	...
	//output_size = output_offset; //Need to check if actual output_size equals to the output_offset and update it
	

	printf("Total time spent to encrypt: %.2f ms\n", total_time);

	clock_gettime(CLOCK_MONOTONIC, &end);
    elapsed_ms = get_time(&start, &end);
	...
	doca_task_free(doca_aes_gcm_task_encrypt_as_task(resources.encrypt_task));
...
// Before freeing the destination buffer, unlock the memory
/*if (munlock(dst_buffer, max_encrypt_buf_size*num_chunks) != 0) {
	DOCA_LOG_ERR("Failed to unpin memory for the destination buffer");
	// Handle error appropriately
}

// Before unmapping and closing, unlock the memory
if (munlock(file_data, file_size) != 0) {
    DOCA_LOG_ERR("Failed to unpin memory for the source buffer");
}*/
