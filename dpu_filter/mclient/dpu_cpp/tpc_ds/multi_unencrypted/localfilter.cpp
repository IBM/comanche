...
void filterDataSync(const arrow::compute::Expression& filter_expression, std::shared_ptr<arrow::io::BufferReader> bufferReader, int row_group_index, std::shared_ptr<Http::ResponseWriter> response, json& aggregation_result, std::shared_ptr<arrow::Table>& final_result_table, const std::vector<std::string>& select_columns, bool is_aggregation) {   try {
...
public:
    FilterHandler(std::vector<char>& data)
        : data(data) {}
            ...
            if (headObjectOutcome.IsSuccess()) {
                ...
                if (success) {
                    ...
                    for (int row_group_index = 0; row_group_index < num_row_groups; ++row_group_index) {
                        futures.push_back(thread_pool.enqueue([=, response_ptr, bufferReader, &final_result_table, &aggregation_result] {
                            filterDataSync(filter_expression, bufferReader, row_group_index, response_ptr, aggregation_result, final_result_table, select_columns, is_aggregation);
                        }));
                    }
                    ...
                    std::cout << "Time to filter: " << filter_duration.count() << " seconds" << std::endl;
                    ...
                    if (is_aggregation) {
                        ...
                        std::chrono::duration<double> write_elapsed = end_write - start_write;
                        std::cout << "Write time taken: " << write_elapsed.count() << " seconds" << std::endl;
...
int main() {
    ...
    auto handler = std::make_shared<FilterHandler>(data_buffer);
