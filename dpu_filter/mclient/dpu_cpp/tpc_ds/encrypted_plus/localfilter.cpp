...
void filterDataSync(const arrow::compute::Expression& filter_expression, 
                    ...
                    int row_group_index, 
                    std::shared_ptr<Http::ResponseWriter> response, 
...
/*
// Read the entire table and filter it at once
void processDataSync(const arrow::compute::Expression& filter_expression, 
                     std::shared_ptr<arrow::io::BufferReader> bufferReader, 
                     std::shared_ptr<Http::ResponseWriter> response, 
                     nlohmann::json& aggregation_result, 
                     std::shared_ptr<arrow::Table>& final_result_table, 
    ...
    try {
        ...
        if (!status.ok()) {
            std::lock_guard<std::mutex> lock(response_mutex);
            response->send(Http::Code::Internal_Server_Error, "Failed to read the table.");
            return;
        }
        ...
        if (req.resource() == "/data" && req.method() == Http::Method::Post) {
            ...
            try {
                ...
                if (getObjectOutcome.IsSuccess()) {
                    ...
                    std::cout << "Filtering time taken: " << filterTime.count() << " seconds" << std::endl;
                    ...
                    if (is_aggregation) {
                        ...
                        if (output == "json") {
                            ...
                            // Create writer properties with compression enabled
                            // Create writer properties with compression enabled
                            ...
                            PARQUET_THROW_NOT_OK(parquet::arrow::WriteTable(
                                *final_table,
                                arrow::default_memory_pool(),
                                buffer_output,
                                /*row_group_size=*/1024 * 1024,  // Optional: size of row groups
                            ...
                            std::ofstream outfile("/tmp/encrypted_output.parquet", std::ios::binary);
                            outfile.write(reinterpret_cast<char*>(encrypted_data), output_size);
                            outfile.close();


                            response_ptr->send(Http::Code::Ok, std::string(reinterpret_cast<char*>(encrypted_data), output_size), MIME(Application, OctetStream));

                            // Code to print the Parquet file
                            auto parquet_reader = std::make_shared<arrow::io::BufferReader>(buffer);
                            std::unique_ptr<parquet::arrow::FileReader> file_reader;
                            auto st = parquet::arrow::OpenFile(parquet_reader, arrow::default_memory_pool(), &file_reader);
                            if (st.ok()) {
                                std::shared_ptr<arrow::Table> table;
                                file_reader->ReadTable(&table);
                                if (table) {
                                    std::cout << "Parquet File Content:\n" << table->ToString() << std::endl;
                    ...
                    std::cout << "Total time taken: " << totalTime.count() << " seconds." << std::endl;
                                    ...
                                    // Log the timing results to file
                if (isFirstQuery) {
