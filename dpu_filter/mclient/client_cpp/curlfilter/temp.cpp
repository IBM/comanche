                ...
                ///////////////////////////////////////////////////////////
                                ...
                                if (useMatchingGroups) {
                                    ...
                                    for (int row_group_index = 0; row_group_index < arrowReader->num_row_groups(); ++row_group_index) {
                                                        ...
                                                        // Prefetching for the next row group
    if (row_group_index + 1 < arrowReader->num_row_groups()) {
        
        auto row_group_metadata = metadata->RowGroup(row_group_index + 1);
        size_t next_row_group_offset = row_group_metadata->file_offset();
        size_t next_row_group_size = row_group_metadata->total_byte_size();
        __builtin_prefetch((const void*)(decrypted_data.data() + next_row_group_offset), 0, 3);
    }        
