...
int main() {
    ...
    while (1) {
        ...
        FILE *fileToForward = tmpfile(); // Create a temporary file to store data
        if (fileToForward == NULL) {
        ...
        while (1) {
            ...
            fwrite(buffer, 1, bytesReadFromServer, fileToForward);
        }

        printf("Received the full file from the server.\n");

        // Rewind the temporary file to the beginning
        rewind(fileToForward);

        // Forward data from the temporary file to the client
        while (1) {
            int bytesReadFromFile = fread(buffer, 1, BUFFER_SIZE, fileToForward);
            if (bytesReadFromFile <= 0) {
                // End of file or error reading from the file
                break;
            }

            // Send data to the client
            int bytesSentToClient = send(clientSock, buffer, bytesReadFromFile, 0);
