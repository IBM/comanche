...
struct Message_pool_response : public Message {
  ...
  Message_pool_response() { 
    assert(this->version == PROTOCOL_VERSION);
  }
  uint64_t pool_id;
} __attribute__((packed));
...
struct Message_IO_request : public Message {
  ...
  void set_key_value_len(size_t buffer_size,
    ...
    if(unlikely((key_len + sizeof(Message_IO_request)) > buffer_size))
      throw API_exception("Message_IO_request::set_key_value_len - insufficient buffer for key-value_len pair (key_len=%lu) (val_len=%lu)",
                          key_len, value_len);
  ...
  void set_key_and_value(const size_t buffer_size,
    ...
    if(unlikely((p_key_len + p_value_len + 2 + sizeof(Message_IO_request)) > buffer_size))
      throw API_exception("Message_IO_request::set_key_and_value - insufficient buffer for key-value pair (key_len=%lu) (val_len=%lu) (buffer_size=%lu)",
