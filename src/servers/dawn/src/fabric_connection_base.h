...
protected:
  ...
  ~Fabric_connection_base() {
    ...
    /* ERROR: RDMA and FABRIC disagree on the name (disconnect vs. close_connection).
     * Maybe RDMA's choice (disconnect) is better. One less word and one less syllable.
     * ERROR: in fabric, you cannot do anything with a connection after you close it.
     * In particular, you cannot deregister memory. So close_connection is the *last"
     * operation on "_transport."
     */
  ...
  bool check_for_posted_value_complete(bool* added_deferred_unlock = nullptr) {
    ...
    if(_posted_value_buffer) { /* release 'value buffer' from get_direct */
      ...
      if(added_deferred_unlock)
        *added_deferred_unlock = true;
  ...
  void post_recv_buffer(buffer_t * buffer) {
    ...
    _transport->post_recv(_posted_recv_buffer->iov,
                          ...
                          &_posted_recv_buffer->desc,
                          _posted_recv_buffer);
  }

  void post_send_buffer(buffer_t * buffer,
