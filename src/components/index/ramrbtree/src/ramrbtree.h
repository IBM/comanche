...
class RamRBTree_factory:public Component::IKVIndex_factory{
	public:
		...
		virtual Component::IKVIndex * create(const std::string& owner, const std::string& name) override{
			Component::IKVIndex *obj=static_cast<Component::IKVIndex*>(new RamRBTree(owner, name));
			assert(obj);
			obj->add_ref();
			return obj;
		}
