...
int read_super(struct super_block *sp)
{
  ...
  if(magic == SUPER_V2 || magic == SUPER_MAGIC) {
	PLOG("MFS: only supports V3 filesystems.\n");
	return -EINVAL;
  }
  ...
  /* Limit s_max_size to INT_MAX */
  if ((unsigned long)sp->s_max_size > INT_MAX)
	sp->s_max_size = INT_MAX;
  ...
  if (sp->s_imap_blocks < 1 || sp->s_zmap_blocks < 1
  	...
  	PLOG("not enough imap or zone map blocks, \n");
  	PLOG("or not enough inodes, or not enough zones, \n"
  ...
  if(sp->s_flags & MFSFLAG_MANDATORY_MASK) {
  	PLOG("MFS: unsupported feature flags on this FS.\n"
