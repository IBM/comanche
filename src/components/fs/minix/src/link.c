...
int fs_link(ino_t dir_nr, char *name, ino_t ino_nr)
{
  ...
  if((new_ip = advance_inode(ip, name)) == NULL) {
	  r = err_code;
	  if(r == -ENOENT)
		  r = OK;
  } else {
              ...
              char *new_name)
{
  ...
  old_ip = advance_inode(old_dirp, old_name);
  r = err_code;
  ...
  new_ip = advance_inode(new_dirp, new_name); /* not required to exist */
  ...
  if(r == OK) {
    ...
    if (odir && !same_pdir) {
      ...
      while (TRUE) {	/* may hang in a file system loop */
        ...
        next_new_superdirp = advance_inode(new_superdirp, "..");
