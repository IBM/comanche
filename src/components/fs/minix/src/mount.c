 ...
 */
  ...
  if(!(superblock.s_flags & MFSFLAG_CLEAN) && !readonly) {
	...
	PLOG("MFS: WARNING: FS 0x%lx unclean, mounting readonly\n", fs_dev);
  }
  ...
  if( (root_ip = get_inode(fs_dev, ROOT_INODE)) == NULL)  {
	PLOG("MFS: couldn't get root inode\n");
	superblock.s_dev = NO_DEV;
	bdev_close(fs_dev);
	return(-EINVAL);
  }
  ...
  if(root_ip->i_mode == 0) {
	PLOG("%s:%d zero mode for root inode?\n", __FILE__, __LINE__);
	put_inode(root_ip);
	superblock.s_dev = NO_DEV;
	bdev_close(fs_dev);
	return(-EINVAL);
...
/* Unmount a file system. */
  ...
  if (count != 1)
	PLOG("MFS: file system has %d in-use inodes!\n", count);
