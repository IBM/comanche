...
class Block_device_component : public Core::Physical_memory,
 ...
 public:
  DECLARE_VERSION(0.1);
  DECLARE_COMPONENT_UUID(0x24518582, 0xd731, 0x4406, 0x9eb1, 0x58, 0x70, 0x26,
                         0x40, 0x8e, 0x23);
 ...
 public:
   ...
   * @return
   */
  Block_device_component(const char *pci_addr, cpu_mask_t *cpus,
                         Core::Poller *poller);
   ...
   */
  virtual workid_t async_read(io_buffer_t buffer, uint64_t offset, uint64_t lba,
                              uint64_t lba_count, int queue_id = 0,
   ...
   * @return Work identifier which is an ordered sequence number
   */
  virtual workid_t async_write(io_buffer_t buffer, uint64_t offset,
   ...
   */
  virtual void attach_work(std::function<void(void *)> work_function, void *arg,
...
class Block_device_component_factory : public Component::IBlock_device_factory {
 public:
  DECLARE_COMPONENT_UUID(0xFAC2215b, 0xff57, 0x4efd, 0x9e4e, 0xca, 0xe8, 0x9b,
                         0x70, 0x73, 0x2a);
  ...
  IBlock_device *create(std::string device_id, cpu_mask_t *cpuset = nullptr,
