 ...
 * @param queue 
 */
static void do_work(struct rte_ring * ring,
                    Nvme_queue * queue)
{
  ...
  if(unlikely(desc->op == COMANCHE_OP_CHECK_COMPLETION)) {
    ...
    if(desc->tag == 0) {
      desc->status = queue->pending_remain() ? IO_STATUS_COMPLETE : IO_STATUS_PENDING;
    }
    else {
      desc->status = queue->check_completion(desc->tag) ? IO_STATUS_COMPLETE : IO_STATUS_PENDING;
    }
  }
  /** submission request */
  else {
...
static struct rte_ring* register_ring(unsigned core, Nvme_device * device)
{
  ...
  struct rte_ring * ring = rte_ring_create(tmp,
                                           Nvme_device::IO_SW_QUEUE_DEPTH,
                                           numa_node_of_cpu(core),
                                           0);
  if(ring == nullptr)
    throw General_exception("rte_ring_create failed for IO thread (name=%s)", tmp);
...
static void* poller_cb_init(unsigned core, void* thisptr)
{
  ...
  arg->queue = this_->allocate_io_queue_pair(this_->DEFAULT_NAMESPACE_ID); /* namespace id */
  assert(arg->queue);
    ...
    _activate_io_threads(false)
{
  ...
  if(!poller)
    throw API_exception("poller reference is null");

  poller->register_percore_task(poller_cb_init, poller_cb, poller_cb_cleanup, this);
                                        ...
                                        int queue_id)
{
  ...
  while((rc = rte_ring_mp_enqueue(ring,(void*) desc)) != 0) {
    ...
    cpu_relax();
    if(retries++ > 100) {
      //      PWRN("push request onto FIFO queue jammed; too much pressure - backed off");
      usleep(100);
    }
  }
}
...
bool Nvme_device::check_completion(uint64_t gwid, int queue_id)
{
  ...
  struct rte_ring * ring = _qm_state.ring_list[queue_id];
  if(!ring)
    throw API_exception("invalid queue for check_completion; default may be invalid");
...
get_serial_hash()
{
  const struct spdk_nvme_ctrlr_data * caps = get_controller_caps();
  assert(caps);
  return CityHash64(reinterpret_cast<const char*>(&caps->sn), SPDK_NVME_CTRLR_SN_LEN);
}

void
Nvme_device::
attach_work(unsigned queue_id, std::function<void(void*)> work_function, void * arg)
{
