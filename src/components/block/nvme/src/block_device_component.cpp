...
Block_device_component::Block_device_component(const char* pci_addr,
                                               cpu_mask_t* cpus,
                                               Core::Poller* poller) {
  if (cpus && poller) throw API_exception("conflicting parameters");
  ...
  if (cpus) {
    ...
    throw API_exception("missing parameters (cpus/poller)");
}

Block_device_component::~Block_device_component() noexcept { delete _device; }

workid_t Block_device_component::async_read(io_buffer_t buffer, uint64_t offset,
                                            uint64_t lba, uint64_t lba_count,
                                            int queue_id, io_callback_t cb,
                                            void* cb_arg0, void* cb_arg1) {
  if (lba_count == 0)
...
workid_t Block_device_component::async_write(io_buffer_t buffer,
                                             uint64_t offset, uint64_t lba,
                                             uint64_t lba_count, int queue_id,
                                             io_callback_t cb, void* cb_arg0,
                                             void* cb_arg1) {
  if (lba_count == 0) throw API_exception("bad async_write param");
...
bool Block_device_component::check_completion(uint64_t gwid, int queue_id) {
...
void Block_device_component::attach_work(
    std::function<void(void*)> work_function, void* arg, int queue_id) {
  _device->attach_work(queue_id, work_function, arg);
}

void Block_device_component::get_volume_info(VOLUME_INFO& devinfo) {
  strncpy(devinfo.volume_name, _device->get_device_id(), VOLUME_INFO_MAX_NAME);

  strncpy(devinfo.device_id, _device->get_pci_id(), VOLUME_INFO_MAX_NAME);
