...
class Nvme_queue {
 ...
 public:
   ...
   */
  Nvme_queue(Nvme_device *device, unsigned qid,
 ...
 private:
  ...
  std::vector<std::pair<std::function<void(void *)>, void *> >
      _shared_work_functions;
 ...
 public:
  ...
  inline void attach_work(std::function<void(void *)> work_function,
                          void *arg) {
    _shared_work_functions.push_back({work_function, arg});
  }

  inline bool pending_remain() { return _pending_hd == nullptr; }
  ...
  inline void push_pending_fifo(IO_descriptor *desc) {
    if (option_DEBUG) PLOG("pushing FIFO: %p (tag=%lu)", desc, desc->tag);

    IO_descriptor *oldnext = nullptr;
    if (_pending_hd) oldnext = _pending_hd;

    _pending_hd = desc;
    desc->next = oldnext;
    if (oldnext) oldnext->prev = desc;
    desc->prev = nullptr;

    /* update tail */
    if (desc->next == nullptr) _pending_tl = desc;
  }
