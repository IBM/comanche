...
TEST_F(Block_nvme_test, PartitionIntegrity)
{
  ...
  _block->write(mem, 0, 0, 1, TEST_QUEUE_ID);
  PLOG("First block write OK.");
  
  for(unsigned k=0;k < BLOCK_COUNT; k++) {
    _block->write(mem, 0, k, 1, TEST_QUEUE_ID);
  }  
  ...
  for(unsigned i=0;i<ITERATIONS;i++) {
    ...
    tag = _block->async_read(mem, 0, lba, 1, TEST_QUEUE_ID);
    ...
    v[0] = lba;
    tag = _block->async_write(mem, 0, lba, 1, TEST_QUEUE_ID);
    while(!_block->check_completion(tag,TEST_QUEUE_ID));
...
TEST_F(Block_nvme_test, WriteLatencyChecked)
{
  ...
  for(unsigned i=0;i<ITERATIONS;i++) {
    ...
    ptr[0] = 0;
    _block->read(mem, 0, i, 1, TEST_QUEUE_ID);
...
TEST_F(Block_nvme_test, CheckAllWriteThroughput)
{
  ...
  for(unsigned i=0;i<ITERATIONS;i++) {
    tags[i] = _block->async_write(mem, 0, i, 1, TEST_QUEUE_ID);
    //    PLOG("issued tag: %ld", tags[i]);
    if(tags[i] % 4096) _block->check_completion(tags[i], TEST_QUEUE_ID);
  }
  for(unsigned i=0;i<ITERATIONS;i++) {
    while(!_block->check_completion(tags[i],TEST_QUEUE_ID));
  }
