 ...
 */
static int __reg_op(PMEMobjpool *pop,
                    ...
                    int          reg_op)
{
  ...
  switch (reg_op) {
    ...
    case REG_OP_ALLOC:
      ...
      TX_BEGIN(pop)
      {
        pmemobj_tx_add_range(bitdata.oid, index * sizeof(long),
                             nlongs_reg * sizeof(long));
        word_t *tab_start = D_RW(bitdata);
        for (i = 0; i < nlongs_reg; i++) tab_start[index + i] |= mask;
      }
      TX_ONABORT
      {
    ...
    case REG_OP_RELEASE:
      ...
      TX_BEGIN(pop)
      {
        pmemobj_tx_add_range(bitdata.oid, index * sizeof(long),
                             nlongs_reg * sizeof(long));
        for (i = 0; i < nlongs_reg; i++) D_RW(bitdata)[index + i] &= ~mask;
      }
      TX_ONABORT
      {
        PERR("%s: transaction aborted: %s\n", __func__, pmemobj_errormsg());
...
int bitmap_tx_zero(PMEMobjpool *pop, TOID(struct bitmap_tx) bitmap)
{
  ...
  TX_BEGIN(pop) { TX_MEMSET(D_RW((D_RW(bitmap)->bitdata)), 0, mem_size); }
  TX_ONABORT
  {
    PERR("%s: transaction aborted: %s\n", __func__, pmemobj_errormsg());
    abort();
                     ...
                     unsigned nbits)
{
  ...
  PINF("%s: bitmap data created at %p", __func__,
       pmemobj_direct((D_RO(bitmap)->bitdata).oid));
   ...
   * contention!*/
  {
    for (offset = 0; offset + step <= nbits;
      ...
      if (!__reg_op(pop, bitmap, pos, order, REG_OP_ISFREE)) {
        PDBG("%s: (%lu) reg not free at %lu", __func__,
...

      {
        ...
        if (!__reg_op(pop, bitmap, pos, order, REG_OP_ISFREE)) {
          PDBG("%s: (%lu) retest: reg not free at %lu", __func__,
               std::hash<std::thread::id>{}(std::this_thread::get_id()), pos);
          continue;
        }
        __reg_op(pop, bitmap, pos, order, REG_OP_ALLOC);
        PDBG("%s: (%lu) set! at %lu", __func__,
             std::hash<std::thread::id>{}(std::this_thread::get_id()), pos);
      }
