...
public:
  ...
  /** 
   * Poll completions and return last completion
   * 
   * 
   * @return Last completion wid
   */
  ...
  /** 
   * Post a send without waiting. The sent is gathered from the implicit
   * memory region belonging to this class instance (normally used 
   * for a header) and an additional memory region passed through (normally
   * used for data.  This function does NOT poll for completion.
   * 
   * @param first_sge_len 
   * @param extra_mr 
   * 
   * @return 
   */
  // status_t post_send_trim(size_t first_sge_len,
  //                         int* complete_count_ptr,
  //                         struct ibv_mr * extra_mr = NULL)
  // {
  //   int rc = channel_post_send_trim(_ctx, (uint64_t) complete_count_ptr, first_sge_len, extra_mr);
  //   if(rc) {
  //     assert(0);
  //     return E_FAIL;
  //   }
  ...
  // status_t post_send_trim(size_t first_sge_len,
	...
	// 		  struct ibv_mr * extra_mr = NULL)
  // {
  //   int rc = channel_post_send_trim(_ctx, wid, first_sge_len, extra_mr);
  //   if(rc) {
  //     assert(0);
  //     return E_FAIL;
  //   }
  ...
  // int post_send_single_mr(size_t mr_size,
  //               int* complete_count_ptr,
  //               struct ibv_mr * mr)
  // {
  //   int rc = channel_post_send(_ctx, (uint64_t) complete_count_ptr, mr, mr_size);
  //   if(rc == 1) _outstanding++;
  //   return rc;
  // }

  // status_t post_send_single_mr(size_t mr_size,
	// 		  uint64_t wid,
	// 		  struct ibv_mr * mr)
  // {
  //   int rc = channel_post_send(_ctx, wid, mr, mr_size);
  //   if(rc) {
  //     PWRN("channel_post_send: errno=%d", rc);
  //     assert(0);
  //     return E_FAIL;
