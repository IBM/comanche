...
pte_t* walk_page_table(u64 addr, unsigned long * huge_pfn)
{
    ...
    if (vma->vm_flags & VM_HUGETLB) {
      /* extract bits 51:12 */
      unsigned long paddr = pmd_val(*pmd) & 0xfffffffe00000UL;
      /* unsigned long paddr = ((pmd_val(*pmd) >> 21) << 21); */
      /* paddr = (paddr << 12) >> 12;  */
      *huge_pfn = paddr;
      return NULL;
    }
    ...
    if(*((u64*)pmd) & (1<<7)) {
      printk(KERN_ERR "walk_page_table: bad PMD; not huge, yet flagged huge");
      return NULL;
    }
