...
#define USE_GCC_CAS_INTRINSIC
#else
inline bool __sync_bool_compare_and_swap(volatile int* dest, int cmp_val, int new_val)
{
  int tmp;
...
class Atomic
{
 ...
 public:
  ...
  inline void init_value(atomic_t a)
  {
    __atomic = a;
  }
  inline atomic_t get_value_unsafe()
  {
    return __atomic;
  }
  ...
  static inline atomic_t compare_and_swap(atomic_t* mem, atomic_t oldval, atomic_t newval)
  {
  ...
  {
    return __sync_bool_compare_and_swap(&__atomic, v.get_value_unsafe(), q.get_value_unsafe());
  }
