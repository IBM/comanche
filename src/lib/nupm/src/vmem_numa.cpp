...
Vmem_allocator::~Vmem_allocator()
{
  for (unsigned s = 0; s < _n_sockets; s++) vmem_delete(_vmm[s]);
}
...
void *Vmem_allocator::alloc(int numa_node, size_t size)
{
  ...
  if (vmm == nullptr) throw API_exception("invalid numa node (%d)", numa_node);

  return vmem_malloc(vmm, size);
}

void Vmem_allocator::free(void *ptr) { vmem_free(vmem_for(ptr), ptr); }
