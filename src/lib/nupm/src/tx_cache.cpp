...
static void __segv_handler_trampoline(int sig, siginfo_t* si, void* context)
{
  ...
  
  {
    ...
    if(i != tx_cache.intervals.end()) {
      ...
      void * np = mmap(page_start,
                       page_size,
                       PROT_READ | PROT_WRITE | PROT_EXEC,
                       MAP_SHARED | MAP_FIXED | MAP_ANONYMOUS | huge_flags,
                       0,
                       0);
      if(np != page_start)
        throw Logic_exception("mapping of physical page failed (%s)",strerror(errno));
      ...
      tx_cache.mapped_page_count++;
      if(tx_cache.debug_level > 2)
        PLOG("[TX]: mapped physical page (%lu) to address %p", page_size, page_start);
      return;
    }
    else {
      if(tx_cache.debug_level > 2)
        PLOG("calling default SIGSEGV handler");
    }
  }
...
__attribute__((constructor)) static void tx_cache_ctor()
{
  ...
  if(tx_cache.debug_level > 2)
    PINF("[TX]: installed new SIGSEGV handler.");
...
void * allocate_virtual_pages(size_t n_pages, size_t page_size, uint64_t hint)
{
  ...
  int huge_flags = 0;
  if(page_size == MB(2))
    huge_flags = MAP_HUGETLB | MAP_HUGE_2MB;
  ...
  void * p = mmap(reinterpret_cast<void*>(hint), /* address hint */
                  ...
                  0); /* offset */
  if(p == nullptr)
    throw Logic_exception("mmap failed unexpectedly");
