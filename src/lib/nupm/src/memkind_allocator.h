...
class Memkind_allocator {
 ...
 public:
  ...
  void add_new_region(addr_t region, size_t length) {
    throw API_exception("add_new_region is not implemented in memkind allocator");
  }
  ...
  void * malloc(size_t size) {
    if(option_DEBUG > 1)
      PLOG("memkind: malloc(size=%lu)", size);
  ...
  int posix_memalign(void **memptr, size_t size, size_t alignment) {
    ...
    if(option_DEBUG > 1)
      PLOG("memkind: memalign(memptr=%p, alignment=%lu, size=%lu)",
