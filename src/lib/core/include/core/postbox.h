...
class Mpmc_postbox {
...
#ifdef __CUDACC__
  static_assert(sizeof(unsigned long long int) == sizeof(T),
                "T must be 64-bit");
#endif
 ...
 public:
   ...
   */
  CUDA_DUAL_MEMBER Mpmc_postbox(void* shared_buffer, size_t shared_buffer_size,
                                bool master = false) {
  ...
  bool collect(T& out_val) {
    ...
    do {
      if (_slots[next] == 0) {
        ...
        // PLOG("goo = %ld", goo);
        if (__sync_bool_compare_and_swap((unsigned long long*) &_slots[next],
  ...
  CUDA_DEVICE_MEMBER bool post_device(T val) {
    ...
    do {
      ...
      else if (atomicCAS((unsigned long long int*) &_slots[next], 0ULL,
  ...
  CUDA_DEVICE_MEMBER bool collect_device(T& out_val) {
    ...
    do {
      if (_slots[next] == 0) {
        ...
        T val = _slots[next];
        if (atomicCAS((unsigned long long int*) &_slots[next],
                      (unsigned long long int) val, 0ULL) == val) {
          out_val = val;
          _next_to_collect = next + 1;
          if (_next_to_collect == _num_slots) _next_to_collect = 0;
          return true;
