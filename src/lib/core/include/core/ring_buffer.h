...
protected:
  ...
  static std::string derive_static_ring_buffer_name() {
    std::string name = "rbuf-" + std::to_string(_static_name_index.fetch_add(1));
    return name;
  }
...
{
public:  
  Ring_buffer(const char * name, size_t size) :
    Ring_buffer_base(name,size) {
    static_assert(sizeof(T)==sizeof(void*), "Ring_buffer unsupported type (size!=sizeof(void*))");
  }
  
  Ring_buffer(std::string name, size_t size) :
    Ring_buffer_base(name,size) {
    static_assert(sizeof(T)==sizeof(void*), "Ring_buffer unsupported type (size!=sizeof(void*))");
  }
  ...
  inline int full() {
    return Ring_buffer_base::full();
  }

  inline int empty() {
    return Ring_buffer_base::empty();
  }
