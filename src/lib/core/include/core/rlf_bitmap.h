 ...
 *      Core::Relocatable_LF_bitmap * slab = new (ptr) Core::Relocatable_LF_bitmap(4096,128);
 */
...
public:
    ...
    : _n_elements(n_elements)
  {
    ...
    if(memory_size < (sizeof(Relocatable_LF_bitmap) + (n_elements / 8)))
      throw API_exception("insufficient memory for requested number of elements");
    ...
    if(init) {
      ...
      if(_magic != MAGIC)
        throw API_exception("reconstruction from memory that doesn't match magic");
    }

    if(option_DEBUG)
      PLOG("Relocatable_LF_bitmap: this=%p,  memory_size=%lu, qwords=%u, n_elements=%u", this, memory_size, _n_qwords, _n_elements);  
  }
  ...
  range_type allocate() {
    ...
    while(curr_qword != _n_qwords) {
      ...
      if(__sync_bool_compare_and_swap(&_bitmap[curr_qword],
                                      old_qword, new_qword)) {
  ...
  void free(range_type slot) {
    ...
    if((_bitmap[bitmap_qword] & (1ULL << offset)))
      throw API_exception("bad slot: %u qword=%lx", slot, _bitmap[bitmap_qword]);
  retry:
    uint64_t old_qword = _bitmap[bitmap_qword];
    uint64_t new_qword = old_qword | (1ULL << offset); /* set bit */
    if(!__sync_bool_compare_and_swap(&_bitmap[bitmap_qword],
                                     old_qword, new_qword)) {
      goto retry;
    }
  }
