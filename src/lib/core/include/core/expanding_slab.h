...
/** 
     ...
     * arrangement.  It is also capable of dynamically expanding.  This class is used
     * as the slab allocator for tree nodes.  Provides base interface (Common::Base_slab_allocator).
     *
     * NOTE: this allocator is NOT thread-safe.
     */
template <typename T, template <typename T> class Element = Slab::__BasicElement>
class Expanding_allocator : public Common::Base_slab_allocator
{
 ...
 public:
  /** 
       * Constructor. 
       * 
       * @param max_slots Desired max # of slots.  May be more due to page size round up.
       */
  Expanding_allocator(size_t max_slots, std::string label, bool exact = false)
    : _region(round_up_page((sizeof(Element<T>) * max_slots) + sizeof(Header)))
    , _max_slots(exact ? max_slots : (round_up_page(sizeof(Element<T>) * max_slots)) / sizeof(Element<T>))
  ...
  void* alloc()
  {
    ...
    if (_free_slots.size() > 0) {
      ...
      if (DEBUG) PDBG("picked up free slot (%p, flags=%x)", (void*) slot, slot->hdr.flags);
      assert(slot);
      ...
      if (DEBUG) PLOG("Expanding allocator: range(%lx,%lx)", _region.base(), _region.base() + _region.mapped_size());
 ...
 public:
  ...
  void __dbg_dump_status()
  {
    ...
    PLOG("      : chksum        (%lx)", Common::chksum32(_header, Element<T> * _header->max_slots));
    PLOG("---------------------------------------------------");
 ...
 private:
  Core::Slab::Lazily_extending_region _region; /**< manages the base contiguous region of memory */
