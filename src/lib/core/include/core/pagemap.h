...
class Pagemap {
 ...
 public:
   ...
   */
  pmentry_t* get_page_entries(addr_t start_page, addr_t end_page,
                              size_t* out_num_entries);

  /**
   * Return soft-dirty state: see
   ...
   * @return True if set
   */
  bool is_soft_dirty(pmentry_t pme) { return pme & (1UL << 55); }
  ...
  void dump() {
    ...
    for (vector<range_t>::iterator i = range_list.begin();
         i != range_list.end(); i++) {
      struct range_t r = *i;
      while (r.start != r.end) {
        printf("PAGE: 0x%lx\n", r.start);
