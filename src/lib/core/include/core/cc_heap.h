...
#include "cc_bad_alloc.h"
#include "cc_persister.h"
...
namespace Core
{
  ...
  template <typename T, typename Persister = persister>
    ...
    public:
      ...
      using deallocator_type = CC_allocator<T, Persister>;
      template <typename U>
        struct rebind
        {
          using other = CC_allocator<U, Persister>;
      ...
      explicit CC_allocator(void *area_, std::size_t size_, Persister p_ = Persister())
        : Persister(p_)
        , _pool(area_, size_)
      {}

      explicit CC_allocator(void *area_, Persister p_ = Persister())
        : Persister(p_)
        , _pool(area_)
      {}
      ...
      ) -> pointer
      {
        ...
        if ( ptr == 0 )
        {
          throw CC_bad_alloc(0, s, sizeof(T));
        }
