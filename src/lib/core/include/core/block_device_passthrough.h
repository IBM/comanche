...
class Block_device_passthrough_impl : public __Base
{
 public:
   ...
   */
  virtual Component::workid_t async_read(Component::io_buffer_t buffer, uint64_t buffer_offset, uint64_t lba,
   ...
   * @return Work identifier which is an ordered sequence number
   */
  virtual Component::workid_t async_write(Component::io_buffer_t buffer, uint64_t buffer_offset, uint64_t lba,
                                          uint64_t lba_count) override
  {
  ...
  /** 
   * Check for completion of a work request AND all prior work requests. This API is thread-safe.
   * 
