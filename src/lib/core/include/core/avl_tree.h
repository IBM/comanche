...
{
  /* utility functions */
  inline static int node_min(int a, int b)
  {
    return (a < b) ? a : b;
  }

  inline static int node_max(int a, int b)
  {
    return (a > b) ? a : b;
  }
 ...
 private:
  ...
  static int rotate_once(AVL_node *&root, dir_t dir)
  {
    ...
    int heightChange = (root->subtree[otherDir]->bf == 0) ? HEIGHT_NOCHANGE : HEIGHT_CHANGE;
  ...
  static AVL_node *insert(AVL_node *n, AVL_node *&root, int &change)
  {
    ...
    if (result != EQ_CMP) {
      ...
      increase = HEIGHT_NOCHANGE; /* a node than compares equal is already in the tree */
  ...
  /** 
     ...
     * @returns 
     */
  static AVL_node *remove(AVL_node *n, AVL_node *&root, int &change, cmp_t cmp)
  {
    if (root == nullptr) {
    ...
    if (result != EQ_CMP) {
      ...
      ret = root;
      if ((root->subtree[LEFT] == nullptr) && (root->subtree[RIGHT] == nullptr)) {
        root   = nullptr;
        change = HEIGHT_CHANGE;
        return ret;
      }
      else if ((root->subtree[LEFT] == nullptr) || (root->subtree[RIGHT] == nullptr)) {
  ...
  bool validate()
  {
    ...
    if (subtree[RIGHT] && subtree[RIGHT]->compare(this) == MAX_CMP) valid = false;
...
/** 
   * An AVL tree. Node allocation is outside of this class.  The memory location for the
   * root pointer is passed into the constructor
   */
template <class T>
class AVL_tree
{
 public:
  ...
  {
    if (root == nullptr) throw Constructor_exception("AVL_tree: requires root pointer space");
...
// }
  ...
  bool validate()
  {
    return AVL_node<T>::validate(*_root);
  }

  /** 
     * Get pointer to the root of the tree (debugging purposes)
     * 
     * @return Pointer to the root of the tree (note this can change) through rebalancing.
     */
  AVL_node<T> **root() const
  {
    return _root;
  }
 ...
 private:
  ...
  static void dump(traversal_order_t order, AVL_node<T> *node, int level = 0)
  {
    ...
    if (order == KEY) {
      ...
      if (verbose) {
        ...
        printf("@[%p] bf=%x left=%p right=%p level=%d\n", node, (unsigned) node->bf, (void *) node->subtree[LEFT],
  ...
  static void __apply_topdown(AVL_node<T> *node, int level, std::function<void(void *)> func)
  {
    if (node == nullptr) {
      return;
    }
