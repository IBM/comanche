...
status_t
Physical_memory::
realloc_io_buffer(Component::io_buffer_t io_mem, size_t size, unsigned alignment)
{
...
status_t
Physical_memory::
free_io_buffer(Component::io_buffer_t io_mem)
{
  void* ptr = reinterpret_cast<void*>(io_mem);
  if (ptr == nullptr) return E_INVAL;

  if(option_DEBUG)
    PLOG("##- freeing DPDK memory: %p", ptr);
...
void
Physical_memory::
unregister_memory_for_io(void* vaddr, size_t len)
{
...
void*
Physical_memory::
virt_addr(Component::io_buffer_t buffer)
{
...
addr_t
Physical_memory::
phys_addr(Component::io_buffer_t buffer)
{
...
size_t
Physical_memory::
get_size(Component::io_buffer_t buffer)
{
