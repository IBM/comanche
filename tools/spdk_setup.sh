...
source "./spdk/common.sh"

if [ -z "$PCI_WHITELIST" ]
then
    echo "Space separated PCI_WHITELIST should be provided, e.g.: PCI_WHITELIST=\"0000:0b:00.0\" ./spdk_setup.sh"
    exit -1
fi
...
function configure_linux_pci {
	...
	grep "PCI_DEVICE_ID_INTEL_IOAT" ./spdk/pci_ids.h \
	| awk -F"x" '{print $2}' > $TMP
	...
	grep "PCI_DEVICE_ID_VIRTIO" ./spdk/pci_ids.h \
	| awk -F"x" '{print $2}' > $TMP
